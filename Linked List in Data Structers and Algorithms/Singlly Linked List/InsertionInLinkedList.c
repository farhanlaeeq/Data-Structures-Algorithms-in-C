...
void main() {
    ...
    //insert a new node at user preffered place
    int insertPosition = 0;
    newnode = (struct node*)malloc(sizeof(struct node));    
    printf("\nEnter the data in the new node: ");
    scanf("%d", &newnode->data);
    newnode->next = NULL;
    printf("Where do you want to insert the new node between 1 and %d : ",index);
    scanf("%d",&insertPosition);
    if(insertPosition <= index && insertPosition >= 0) {
    temp = head;
    index = 0;
    while (temp != NULL) {
        index++;
    ...
    while (temp != NULL) {
       ...
       printf("Your preffered position is out of range!");
    }
}